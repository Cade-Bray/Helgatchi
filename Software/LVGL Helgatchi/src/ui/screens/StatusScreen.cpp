...
// Key handler for the status screen.
static void status_key_cb(lv_event_t *e) {
...
lv_obj_t* create_status_screen() {
  // Build the standard screen layout.
  ScreenLayout layout = screen_layout_create_standard();
  screen_layout_set_status_text(layout, "Menu", "Status", "");

  // Configure the content container for the status page.
  ...
  lv_label_set_text(row1, "Battery: 78%");

  lv_obj_t *row2 = lv_label_create(content);
  lv_label_set_text(row2, "Uptime: 01:23:45");

  lv_obj_t *row3 = lv_label_create(content);
  lv_label_set_text(row3, "Devices: 12");
  ...
  if (group) {
    ...
    lv_obj_add_event_cb(focus, status_key_cb, LV_EVENT_KEY, nullptr);
  }
